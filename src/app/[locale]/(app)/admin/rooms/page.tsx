...
export default async function RoomsPage({ searchParams }: { searchParams: Promise<{ site?: string }> }) {
  ...
  const rooms = site ? await listRooms(actor, site.id, { includeInactive: true }) : [];
  ...
  return (
    <AdminShell actor={actor} currentSiteId={site?.id}>
      <div className="flex items-center justify-between mb-4">
        ...
        <nav className="flex gap-3 text-sm">
          {sites.map((s) => (
            <Link
              key={s.id}
              href={{ pathname: "/admin/rooms", query: { site: s.id } }}
      ...
      {site && <RoomsManager siteId={site.id} rooms={rooms} />}
    </AdminShell>
  );
}
